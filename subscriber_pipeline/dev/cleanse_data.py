...
logging.basicConfig(filename="./subscriber_pipeline/dev/cleanse_db.log", # "./dev/cleanse_db.log"
...
def test_for_path_id(students, career_paths):
    ...
    try:
        ...
        print('All career_path_id(s) are present.')
...
def test_nulls(df):
    ...
    try:
        ...
        print('No nulls rows found.')

def test_num_cols(local_df, db_df):
    """
    Unit test to ensure that the number of columns in the cleaned Dataframe match 
    the number of columns in the SQLite3 database table.
...
def main():
    ...
    if len(new_missing_data) > 0:
        sqlite_connection = sqlite3.connect('./subscriber_pipeline/dev/cademycode_cleansed.db') # ./dev/cademycode_cleansed.db
    ...
    if len(clean_new_students) > 0:
        ...
        con = create_engine('sqlite:///./subscriber_pipeline/dev/cademycode_cleansed.db', echo=True) # sqlite:///./dev/cademycode_cleansed.db
        ...
        clean_db.to_csv('./subscriber_pipeline/dev/cademycode_cleansed.csv') # ./dev/cademycode_cleansed.csv
        ...
        with open('./subscriber_pipeline/dev/changelog.md', 'w') as f: # ./dev/changelog.md
            for line in w_lines:
                f.write(line)
    else:
        print("No new data")
